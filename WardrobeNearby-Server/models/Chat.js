...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (participantId === req.user.id) return res.status(400).json({ message: 'Cannot create chat with yourself' });

    const existingChat = await Chat.findOne({
      participants: { $all: [req.user.id, participantId] },
      relatedItem: itemId,
    }).populate('participants', 'name profileImage').populate('relatedItem', 'name imageUrl');

    if (existingChat) return res.json(existingChat);

    const newChat = new Chat({ participants: [req.user.id, participantId], relatedItem: itemId || null, messages: [] });
    await newChat.save();
    const populatedChat = await Chat.findById(newChat._id).populate('participants', 'name profileImage').populate('relatedItem', 'name imageUrl');
...
// ... (rest of the file: mark-read, typing routes)
