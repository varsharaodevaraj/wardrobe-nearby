...
const ItemCard = React.memo(({ item, onDelete, onToggleAvailability }) => {
  ...
  const isOwner =
    user?.id === (typeof item.user === "object" ? item.user._id : item.user);
  const ownerName = typeof item.user === "object" ? item.user.name : "Owner";
  const ownerStatus =
    typeof item.user === "object" ? item.user.status : "regular";
  ...
  const isUnreliableUrl =
    featuredImage && featuredImage.includes("placehold.co");
  const displayUrl =
    isUnreliableUrl || imageError ? fallbackImageUrl : featuredImage;
      ...
      <View style={styles.infoContainer}>
        <View style={styles.headerContainer}>
          ...
          <View
            style={[
              styles.listingBadge,
              item.listingType === "sell" && styles.sellBadge,
            ]}
          >
            <Text
              style={[
                styles.listingText,
                item.listingType === "sell" && styles.sellText,
              ]}
            >
        ...
        {item.totalReviews > 0 && (
          <View style={styles.ratingContainer}>
            ...
            <Text style={styles.ratingText}>
              ...
              {item.totalReviews === 1 ? "review" : "reviews"})
            </Text>
          </View>
        )}
        ...
        <View style={styles.statusContainer}>
          ...
          >
            <Ionicons
              name={
                item.isAvailable !== false ? "checkmark-circle" : "close-circle"
              }
              size={12}
          ...
          {isOwner && onToggleAvailability && (
            <Switch
              ...
              onValueChange={(newValue) =>
                onToggleAvailability(item._id, newValue)
              }
