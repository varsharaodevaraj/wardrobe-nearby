  ...
  const handleSubmitRequest = async (customMessage = "") => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert(
          "Request Sent! 🎉", 
          `Your ${requestType} for "${item.name}" has been sent and a message has been added to your chat.`,
          [
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <ScrollView ref={scrollViewRef} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.statusContainer}>
            <View style={[styles.statusBadge, itemData.isAvailable ? styles.availableBadge : styles.unavailableBadge]}>
              <Text style={[styles.statusText, itemData.isAvailable ? styles.availableText : styles.unavailableText]}>
                {itemData.isAvailable ? 'Available' : 'Not Available'}
              </Text>
            </View>
            <View style={styles.listingTypeBadge}>
              <Text style={styles.listingTypeText}>{itemData.listingType === 'rent' ? 'For Rent' : 'For Sale'}</Text>
            </View>
          </View>
          ...
          {itemData.reasonForSelling && itemData.reasonForSelling.trim() && (
            <View style={styles.card}>
              <Text style={styles.sectionTitle}>{itemData.listingType === 'rent' ? 'Reason for Renting' : 'Reason for Selling'}</Text>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.ownerInfo}>
              <View style={styles.ownerAvatar}>
                <Text style={styles.ownerInitial}>{item.user?.name ? item.user.name.charAt(0).toUpperCase() : 'U'}</Text>
              </View>
              <View>
                <Text style={styles.ownerName}>{item.user?.name || 'Unknown User'}</Text>
                {item.user?.status === 'super-lender' && <Text style={styles.superLenderText}>Super Lender</Text>}
              </View>
            </View>
          </View>
          ...
          {isOwner && (
            <View style={styles.card}>
              ...
              <View style={styles.availabilityToggle}>
                <Text style={styles.toggleLabel}>Available for {itemData.listingType}</Text>
                <Switch
          ...
          <View ref={reviewsSectionRef} style={styles.card}>
            ...
            <ReviewsList
              ...
              onEditReview={(review) => { setEditingReview(review); setShowReviewForm(true); }}
      ...
      {!isOwner && (
        <View style={styles.footer}>
          ...
          <TouchableOpacity 
            style={[styles.rentButton, (!itemData.isAvailable || loading) && styles.rentButtonDisabled]} 
            onPress={handleRentNow} 
            disabled={!itemData.isAvailable || loading}
          >
            {loading ? <ActivityIndicator color="white" /> : <Text style={styles.rentButtonText}>{itemData.isAvailable ? (item.listingType === 'sell' ? 'Buy Now' : 'Rent Now') : 'Unavailable'}</Text>}
          </TouchableOpacity>
        </View>
      )}
      ...
      <Modal visible={showReviewForm} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => {setShowReviewForm(false); setEditingReview(null);}}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#f8f9fa' }}>
          <ReviewForm
            itemId={item._id}
            existingReview={editingReview}
            onSuccess={() => { setShowReviewForm(false); setEditingReview(null); setReviewsRefreshTrigger(p => p + 1); }}
...
const styles = StyleSheet.create({
  ...
  imageIndicatorContainer: { flexDirection: 'row', justifyContent: 'center', paddingVertical: 10 },
  imageIndicator: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#E0E0E0', marginHorizontal: 4 },
  ...
  listingTypeBadge: { backgroundColor: '#F3E5F5', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 15 },
  ...
  sectionTitle: { fontSize: 18, fontWeight: '600', color: '#34495e', marginBottom: 10 },
  description: { fontSize: 16, color: '#34495e', lineHeight: 24 },
  reasonText: { fontSize: 16, color: '#7f8c8d', fontStyle: 'italic', lineHeight: 24 },
  
  ownerInfo: { flexDirection: 'row', alignItems: 'center' },
  ownerAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#E0BBE4', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  ownerInitial: { color: '#4A235A', fontSize: 18, fontWeight: 'bold' },
  ownerName: { fontSize: 16, fontWeight: '600', color: '#2c3e50' },
  superLenderText: { fontSize: 12, color: '#B7950B', fontWeight: 'bold', marginTop: 2 },
  
  availabilityToggle: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  toggleLabel: { fontSize: 16, color: '#34495e' },
  
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 20, borderTopWidth: 1, borderTopColor: '#E9ECEF', backgroundColor: 'white', position: 'absolute', bottom: 0, width: '100%' },
  ...
  rentButton: { backgroundColor: '#957DAD', paddingVertical: 12, paddingHorizontal: 24, borderRadius: 30, flexDirection: 'row', alignItems: 'center' },
  rentButtonDisabled: { backgroundColor: '#CED4DA', opacity: 0.8 },
  rentButtonText: { color: 'white', fontSize: 16, fontWeight: 'bold' },
  requestedButton: { backgroundColor: '#27AE60', paddingVertical: 12, paddingHorizontal: 24, borderRadius: 30, flexDirection: 'row', alignItems: 'center' },
  requestedButtonText: { color: 'white', fontSize: 14, fontWeight: 'bold', marginLeft: 6 },
  
  ownerFooter: { padding: 20, borderTopWidth: 1, borderTopColor: '#E9ECEF', backgroundColor: 'white', position: 'absolute', bottom: 0, width: '100%' },
