...
const MyRentalsScreen = ({ navigation }) => {
  const { user } = useAuth();
  const { incomingRequestsCount, loadAllRentalStatuses } = useRental();
        ...
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Error",
                error.message || "Could not revoke request."
              );
            }
          },
        },
    ...
    return (
      <View key={request._id} style={styles.requestCard}>
        <View style={styles.requestHeader}>
          <Image
            source={{
              uri: request.item?.imageUrl || "https://via.placeholder.com/100",
            }}
          ...
          <View style={styles.requestInfo}>
            ...
            <View style={styles.statusContainer}>
              <View
                style={[styles.statusBadge, styles[`status_${request.status}`]]}
              >
                <Text
                  style={[
                    styles.statusText,
        ...
        {request.status === "pending" && isOutgoing && (
          <View style={styles.actionButtons}>
            <TouchableOpacity
              ...
              onPress={() => handleRevokeRequest(request._id)}
            >
              <Ionicons name="close-circle" size={18} color="white" />
              <Text style={styles.actionButtonText}>Revoke Request</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {canReview && (
          <View style={styles.actionButtons}>
            <TouchableOpacity
              ...
              onPress={() => navigateToReview(request)}
            >
              <Ionicons name="star-half" size={18} color="white" />
              <Text style={styles.actionButtonText}>Review User</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          ...
          >
            History ({acceptedRentals.length})
          </Text>
        </TouchableOpacity>
      </View>
